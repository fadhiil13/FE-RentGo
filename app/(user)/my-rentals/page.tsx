...
import { Calendar, Car, RefreshCw, CreditCard, FileText, Star, XCircle, Loader2 } from 'lucide-react'
          ...
          <div className="space-y-4">
            {rentals.map((rental) => (
              <div key={rental.id} className="bg-white/[0.04] border border-white/10 hover:border-[#4ade80]/20 hover:bg-white/[0.06] transition-all rounded-2xl p-5 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 flex-wrap mb-2">
                      ...
                      <span className={`text-xs font-semibold px-3 py-1 rounded-full w-28 text-center flex-shrink-0 ${statusStyle[rental.status] ?? statusStyle.COMPLETED}`}>
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-3 pt-3 sm:pt-0 border-t sm:border-0 border-white/[0.06] flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      {rental.status === 'CONFIRMED' && (
                        <Link href={`/payment/${rental.id}`} className="flex items-center gap-1.5 bg-[#4ade80] hover:bg-[#22c55e] text-[#080f1a] font-bold text-xs px-3 py-1.5 rounded-xl transition-colors whitespace-nowrap">
                          <CreditCard size={12} /> Bayar
                        </Link>
                      )}
                      {['CONFIRMED', 'ONGOING', 'COMPLETED'].includes(rental.status) && (
                        <Link href={`/invoice/${rental.id}`} className="flex items-center gap-1.5 text-xs text-white/40 hover:text-[#4ade80] border border-white/10 hover:border-[#4ade80]/30 px-3 py-1.5 rounded-xl transition-all whitespace-nowrap">
                ...
                {rental.status === 'PENDING' && (
                  <div className="border-t border-white/[0.06] pt-4">
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-white/30">Menunggu konfirmasi dari admin</p>
                      <button
                        onClick={() => handleCancel(rental.id)}
                        disabled={cancellingId === rental.id}
                        className="flex items-center gap-1.5 text-xs text-red-400 border border-red-400/25 hover:bg-red-400/10 px-3 py-1.5 rounded-xl transition-all disabled:opacity-50"
                      >
                        {cancellingId === rental.id
                          ? <><Loader2 size={12} className="animate-spin" /> Membatalkan...</>
                ...
                {/* ── Tombol / Form Review (hanya COMPLETED) ── */}
